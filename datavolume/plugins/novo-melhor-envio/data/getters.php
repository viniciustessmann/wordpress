...
function getCotacao($id)
{
    ...
    $args = array(
        ...
        'headers'=>[
            ...
            'authorization' => base64_encode(get_option('me_api_key').':'.get_option('me_token'))
        ],
...
function getCotacaoFront($pck)
{
    ...
    foreach ($pck['contents'] as $item){
      $product_id = ( 0 < $item['variation_id'] ) ? $item['variation_id'] : $item['product_id'];
        ...
        $width  += wc_get_product( $product_id )->get_width();
        $height += wc_get_product( $product_id )->get_height();
        $length += wc_get_product( $product_id )->get_length();
        $weight += wc_get_product( $product_id )->get_weight()  * $item['quantity'];

        $valor = wc_get_product( $product_id )->get_price() * $item['quantity'];
        $volume  = $volume +  (int) ($width * $length * $height) * $item['quantity'];

        $total += $valor ;

        $log = new WC_Logger();
        $log->add( 'me_test', 'Product ID: ' . $product_id . '. Peso: ' . $weight );
    }
    ...
    //Monta o from
    $from = new stdClass();
    $remetente = $remetente = apply_filters( 'woocommerce_melhorenvio_remetente', getRemetente(), $pck );
    ...
    $log->add( 'melhorenvio-3', print_r( $request, true ) );
    $log->add( 'melhorenvio-3', print_r( $response['body'], true ) );
